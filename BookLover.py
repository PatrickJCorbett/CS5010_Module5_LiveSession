...
class BookLover:
    ...
    def addBook(self, bookName, rating):
        ...
        #put the books already read into a list, will check whether the new book 
        #has already been read or not
        books_already_read = list([x[0] for x in self.bookLst])
        if bookName in books_already_read:
